  ...
  return (
    <div className="mt-2 rounded-lg border border-zinc-800 bg-zinc-950/80 px-3 py-2 text-[11px] text-zinc-400">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <span>{t("approvalGate.allowance")}</span>
        <span className="font-mono text-zinc-200">
          {formatToken(allowanceWei, tokenDecimals, 6)} {symbol}
        </span>
      </div>
      {need ? (
        <div className="mt-1 text-amber-200/90">{t("approvalGate.required")}</div>
      ) : (
        <div className="mt-1 text-emerald-200/85">{t("approvalGate.sufficient")}</div>
      )}
    </div>
  );
}
