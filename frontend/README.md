...
Copy `.env.example` to `.env.local` and update addresses/RPC as needed:

```bash
cp .env.example .env.local
```

Contract-related envs used by the frontend:

- `NEXT_PUBLIC_DUAL_STAKING_ADDRESS`
- `NEXT_PUBLIC_TOKEN_A_ADDRESS`
- `NEXT_PUBLIC_TOKEN_B_ADDRESS`
- `NEXT_PUBLIC_TIMELOCK_CONTROLLER_ADDRESS`（48h 参数治理）
- `NEXT_PUBLIC_TIMELOCK_SUPER_CONTROLLER_ADDRESS`（72h 超级路径）
...
- `NEXT_PUBLIC_STAKING_DEPLOY_BLOCK`
